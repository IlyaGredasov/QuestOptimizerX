...
parser.add_argument(
    "-d", type=float, help="Density of graph (count of edges)", required=True
)
parser.add_argument("-q", type=int, help="Number of quest lines", required=True)
parser.add_argument(
    "-ql", type=int, help="Maximum length of quest line", required=False, default=10
)
...
with open(args.o, "w", newline="") as f:
    ...
    all_edges = [
        (i, j)
        for i in range(args.v)
        for j in range(i + 1 if args.b else 0, args.v)
        if i != j
    ]
    ...
    for edge in edges:
        f.write(
            f"\t{edge[0]} {edge[1]} {f'{100 * random():.2f}' if args.w else ''}{linesep}"
        )
    ...
    for i, quest_line in enumerate(quest_lines):
        f.write(
            "\t"
            + " ".join(map(str, quest_line))
            + (f" quest_name_{i}" if args.qn and random() < 0.5 else "")
            + linesep
        )
